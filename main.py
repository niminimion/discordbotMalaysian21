...
from card_renderer import render_hand_image, render_cards_image
...
async def on_ready() -> None:
    ...
    # Guild-only sync: instant propagation, no duplicates
    # (global commands were cleared in a previous run; decorators still populate in-memory tree)
...
def build_poker_board_embed(table: PokerTable, *, reveal: bool = False) -> discord.Embed:
    ...
    # Community cards — shown as emoji text; image attached separately by _post_board
...
# ---------------------------------------------------------------------------
# Texas Hold'em — Settlement helpers
# ---------------------------------------------------------------------------
...
async def resolve_poker_showdown(
    ...
    # Disable the game view + reveal board
...
class PokerGameView(ui.View):
    ...
    async def _handle_result(
        ...
        if result == "round_over":
            ...
            # New betting round — update board with community image
            await interaction.response.defer()
    ...
    async def check_call_btn(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        if to_call <= 0:
            ...
            if gold <= 0:
                ...
                return
            if gold < to_call:
                # Auto all-in when short on gold
    ...
    async def on_timeout(self) -> None:
        ...
        # Refund all active players their total contributions
        table.phase = "showdown"
        for p in table.players:
            if p.total_in_pot > 0:
...
class PokerLobbyView(ui.View):
    ...
    async def _start_game(self, interaction: discord.Interaction) -> None:
        ...
        # Check debt status before deducting blinds
        for p in table.players:
            if get_gold(table.guild_id, p.user_id) <= 0:
                p.in_debt = True

        # Deal cards and post blinds (amounts returned for gold deduction)
        ...
        # Send initial board (no community cards yet — preflop)
        ...
        # DM each player their hole cards (ephemeral followups always go to the
        # interaction trigger user, not the mentioned player)
        for p in table.players:
...
class PokerRematchView(ui.View):
    ...
    async def rematch_btn(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        # All voted — start game
        self.started    = True
        button.disabled = True
        self.stop()

        # Pick host randomly from available players
        eligible = [
            (uid2, name)
            for uid2, name in self.participants
            if uid2 not in active_player_ids
...
@bot.tree.command(name="poker", description="Start a Texas Hold'em poker table")
@app_commands.describe(blind="Small blind in Gold (big blind = 2×). Default 50.")
async def cmd_poker(interaction: discord.Interaction, blind: int = 50) -> None:
    if not interaction.guild:
        await interaction.response.send_message("This command can only be used in a server.", ephemeral=True)
        return
...
@bot.tree.command(name="pokerstop", description="Close your open Texas Hold'em lobby (host only)")
async def cmd_pokerstop(interaction: discord.Interaction) -> None:
    if not interaction.guild:
